<div class="col-md-6 center">
    <h3 style="color: darkgreen"><u>Your Last <?php echo $historyInPage; ?> Attacks</u></h3>
    <?php
    $hID = 0;

    $sql = "SELECT * FROM history WHERE attackerId = '$user_id' ORDER BY hId DESC";
    $results = mysqli_query($conn,$sql) or die(mysqli_error($conn));
    while($row = $results->fetch_assoc()){
        if($hID < $historyInPage) {
            // PARAMETERS
            $userAttackedId = $row['attackedId'];
            ...
            <div class="accordion history-container" id="history">
                <div class="center" data-toggle="collapse" data-target="#collapse<?php echo $historyId; ?>"
                     aria-expanded="true" aria-controls="collapseOne">
                    <table class="history-table">
                        <tr>
                            <td><i class="fas fa-history"></i></td>
                            <td><?php echo getArmyNameById($userAttackedId); ?></td>
                            <td><?php
                                if ($winnerId == $user_id) {
                                    ?>
                ...
                <div id="collapse<?php echo $historyId; ?>" class="collapse" aria-labelledby="headingOne"
                     data-parent="#history">
                    <div class="card-body">
                        <u>Results:</u>
